...
public class ServiceController {
	...
	public List<Quotation> qTextSearch(String searchExpression) {
		...
		return em.createQuery("select q from Quotation q where q.quoteText like ?1", Quotation.class)
				.setParameter(1, searchExpression).setMaxResults(40).getResultList();
	}
	...
	public List getAutocompleteValues(String searchExpression, String searchType) {
		if (searchType.equals("author")) {
			...
			// em.createQuery("select a from Author a where upper(a.lastName)
			// like ?1").setParameter(1,
			// "%"+searchExpression+"%").getResultList();
	...
	public List getD3TagBubbleData() {
		...
		List<Object[]> tempresult = em
				.createQuery("select s.tagText, size(s.taggedQuotes) from SubjectTag s", Object[].class)
				.getResultList();
	...
	public Object[] getD3TagForceData() {
		...
		List<SubjectTag> result = em
				.createQuery("Select s from SubjectTag s where size(s.taggedQuotes)>13 order by s.id", SubjectTag.class)
				.getResultList();
		...
		for (int i = 0; i < result.size(); i++) {
			...
			List<Integer> linkResult = em
					.createQuery(
							"Select ss.id from SubjectTag s inner join s.taggedQuotes q inner join q.tags ss where s.id=?1 and not(ss.id=?1) and size(ss.taggedQuotes)>13 and ss.id>s.id")
					.setParameter(1, result.get(i).getId()).getResultList();
			...
			for (Integer target : linkResult) {
				...
				if (!links.contains(candidate))
					...
					links.get(links.indexOf(candidate)).count++;
			}
		}
	...
	@RequestMapping(path="/signupval/email")
	public String emailValidation(String email) {
		
		if(!Pattern.matches("...*@.+\\..+", email)){
			return "That email address is not valid";
		}
		else if (!em.createQuery("select u.email from User u where u.email=?1", String.class).setParameter(1, email)
	...
	@RequestMapping("/signupval/username")
	public String usernameValidation(String username){
		if (username==null||username=="")
			return "A username is required";
		else if (em.createQuery("select u from User u where u.username=?1",String.class).setParameter(1, username).getResultList().isEmpty()){
			return "g2g";
		}
		else return "That username is not available";
	}
	
	@Transactional
	@RequestMapping("/newUser")
	public int createUser(User candidate){
		...
		candidate.setTrustFactor(1);
		if (usernameValidation(candidate.getUsername()).equals(emailValidation(candidate.getEmail()))&&candidate.getPassword()!=null){
